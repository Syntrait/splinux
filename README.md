...
We will be utilizing the ```--bind``` argument. If we do ```--bind ~/savedata_location_of_player_2 ~/savedata```, the program that we will be running using bwrap will think that it's reading and writing to ```~/savedata```, however, the operations are actually being redirected to ```~/savedata_location_of_player_2```. This means the game won't end up overwriting player 1's savedata, instead, it will have its own save data location. Pretty cool, right?
